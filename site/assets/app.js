...
if (classMatch) {
  try { localStorage.setItem('eft-last-class', classMatch[1]); } catch { /* private window */ }
}
...
// bar once there is something in it. The class list is stamped onto the element
// by the build, so sixteen titles are never duplicated here.
...
if (strip) {
  ...
  let lastClass = 0;
  try { lastClass = Number(localStorage.getItem('eft-last-class')) || 0; } catch { /* private window */ }
  ...
  if (!done && !lastClass) {
    card = `<div class="next-up">
      ...
      <span class="next-up-s">No prerequisites beyond arithmetic. It ends with the one sentence the
        other fifteen classes spend their bench time proving.</span>
      <a class="btn btn-primary" href="/class/1">Open Class 1</a></div>`;
  } else if (where) {
    card = `<div class="next-up">
      <span class="next-up-t">Class ${where.n} · ${where.title}</span>
      ...
      <a class="btn btn-primary" href="/class/${where.n}">Continue</a></div>`;
  } else {
    card = `<div class="next-up">
      <span class="next-up-t">All ${total} classes marked as studied</span>
      <span class="next-up-s">The useful thing now is retrieval and bench time, not rereading.</span>
      <a class="btn btn-primary" href="/practice#drill">Practise what is due</a></div>`;
  }

  strip.innerHTML = card + (done ? `<div class="pbar">
    <span class="pbar-lbl">${done} of ${total} classes</span>
...
if (prepBlocks.length) {
  ...
  try {
    const last = Number(localStorage.getItem('eft-last-class')) || 0;
